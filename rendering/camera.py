...
class Camera:
    ...
    def __init__(self, mg, cell_size, e_hmap):
        self.mg = mg
        self.cell_size = cell_size
        self.e_hmap = e_hmap
        self.campos = V2()
        self.rcam = pygame.Rect()
        self.world_size = V2(mg.nx, mg.ny)


    def get_dpix(self):
        return (self.campos-rcam.size)*self.cell_size
##        return (camposx - rcam.x)*CELL_SIZE, (camposy - rcam.y)*CELL_SIZE
    ...
    def set_mg_pos_from_rcam(self):
        mg.current_x = int(self.rcam.x)
        mg.current_y = int(self.rcam.y)

    def set_campos_from_rcam(self):
        self.campos = self.rcam.topleft
...
##        camposy = float(rcam.top)
    ...
    def set_rcam_from_rmouse(self):
        ...
        self.rcam.topleft = (V2(self.rmouse)-rminimap.topleft)*self.world_size/rminimap.size
...
##        rcam.y = (rmouse.y - rminimap.y)*WORLD_SIZE[1]/MINIMAP_SIZE[1]
    ...
    def set_rmouse_from_rcam(self):
        rminimap = self.e_hmap.get_rect()
        self.rmouse.topleft = self.rcam.topleft*rminimap.size/self.world_size + rminimap.topleft
